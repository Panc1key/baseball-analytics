...
export const MLB_INFERENCE_FREEZE = Object.freeze({
  ...
  shadowOnly: Object.freeze([
    ...
    'MlbHighEvShrinkShadow shrink_w15_l15（預設 compare；不進正式可看選邊）',
    'MlbWinrateStrongHomeShadow（正式 apply：客+hwp≥62%+EV≥10% skip；回退 compare|off）',
    'MlbSurgicalAwayStrongEvShadow surgical_a（預設 off；不進正式）',
    'MlbSurgicalAwayR1MidoddsShadow surgical_b（正式 apply：客×R1×中水；回退 compare|off）',
    'MlbTotalsFragileUnderShadow（正式 apply：Under×ERA≥5；回退 compare|off）',
    'MlbTotalsUnderBlowupGapShadow（正式 apply：Under×blowup×gap<0.8；回退 compare|off）',
    'MlbTotalsUnderPitcherShadow（正式 apply：Under×投手公園；回退 compare|off）',
    'MlbDirectionBlendDisagreeShadow（僅 compare；禁止 apply）',
    'MlbLockedBPackage totals/parlay 組合包展示（大小不寫入 mlb_paper_bets；不混獨贏 TopK）',
