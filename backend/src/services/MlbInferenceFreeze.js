...
export const MLB_INFERENCE_FREEZE = Object.freeze({
  ...
  shadowOnly: Object.freeze([
    ...
    'MlbSurgicalAwayStrongEvShadow surgical_a（高EV×客×hwp≥65%；預設 off，已由 WinrateStrongHome hwp≥62% 承接；回退/回測 MLB_SURGICAL_AWAY_STRONG_EV_SHADOW=compare|apply；只動獨贏）',
    'MlbSurgicalAwayR1MidoddsShadow surgical_b（客×R1×中水1.95-2.10；預設 apply；回退 MLB_SURGICAL_AWAY_R1_MIDODDS_SHADOW=compare|off；只動獨贏）',
    'MlbTotalsUnderPitcherShadow（Under×投手公園；預設 apply；與 FragileUnder ERA≥5 互補；回退 MLB_TOTALS_UNDER_PITCHER_SHADOW=compare|off；只動大小 Hybrid）',
