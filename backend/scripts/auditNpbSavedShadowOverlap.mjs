/**
 * NPB 已保存影子 vs 基線：重疊／分歧場帳（研究）
 * baseline = logistic mid；shadow = ridge_poisson_cal_same
 *
 * 用法: node scripts/auditNpbSavedShadowOverlap.mjs
 * 產物: tmp-npb-saved-shadow-overlap.json
 */
import fs from 'fs';
...
const SHADOW = NPB_RESEARCH_SHADOW_SPEC.savedPrimaryShadow;
...
function toCand(pHome, row, source) {
...
/** @type {Map<string, { baseline?: object, shadow?: object }>} */
...
for (const hold of months.filter((m) => !warmup.has(m))) {
  ...
  for (const row of labeled) {
    ...
    const slot = byGame.get(row.g.id) || {};
    ...
    if (ridge.ok) {
      ...
      const calP = applyAsianLogisticCalibration(rawP, {
        fromLogit: false,
        shrink: SHADOW.shrink,
        temp: SHADOW.temp,
        fairHome: row.mkt.fairHome,
      });
      const cand = toCand(calP, row, 'shadow');
      if (passGate(cand)) slot.shadow = cand;
    }
    ...
    if (logistic.ok) {
      ...
      const cand = toCand(pHome, row, 'baseline');
      if (passGate(cand)) slot.baseline = cand;
    }

    if (slot.baseline || slot.shadow) byGame.set(row.g.id, slot);
  }
}
...
for (const [, slot] of byGame) {
  const b = slot.baseline;
  const s = slot.shadow;
  if (b && s) {
    if (b.pickHome === s.pickHome) bothSameSide.push({ baseline: b, shadow: s });
    else bothFlipSide.push({ baseline: b, shadow: s });
  } else if (b && !s) baselineOnly.push(b);
  else if (s && !b) shadowOnly.push(s);
}

function pnlFromPair(pairs, which) {
  return summarize(pairs.map((p) => p[which]));
}

const out = {
  researchOnly: true,
  wiredToFormal: false,
  savedShadow: SHADOW.id,
  baseline: BASE.id,
  counts: {
    gamesWithEither: byGame.size,
    bothSameSide: bothSameSide.length,
    bothFlipSide: bothFlipSide.length,
    baselineOnly: baselineOnly.length,
    shadowOnly: shadowOnly.length,
  },
  ledgers: {
    baselineAll: summarize([...baselineOnly, ...bothSameSide.map((p) => p.baseline), ...bothFlipSide.map((p) => p.baseline)]),
    shadowAll: summarize([...shadowOnly, ...bothSameSide.map((p) => p.shadow), ...bothFlipSide.map((p) => p.shadow)]),
    overlapSameSide_baseline: pnlFromPair(bothSameSide, 'baseline'),
    overlapSameSide_shadow: pnlFromPair(bothSameSide, 'shadow'),
    overlapFlip_baseline: pnlFromPair(bothFlipSide, 'baseline'),
    overlapFlip_shadow: pnlFromPair(bothFlipSide, 'shadow'),
    baselineOnly: summarize(baselineOnly),
    shadowOnly: summarize(shadowOnly),
  },
  interpretation: {
    liftSource:
      shadowOnly.length && summarize(shadowOnly).usd50 > 0
        ? 'shadow_only_positive_is_main_volume_lift_candidate'
        : 'check_overlap_and_flip',
    caution:
      bothFlipSide.length > 0
        ? '兩邊都過閘但選邊相反時，以各自帳為準；升格前須人工看翻邊品質'
        : '無翻邊重疊',
  },
};

const path = new URL('../tmp-npb-saved-shadow-overlap.json', import.meta.url);
fs.writeFileSync(path, JSON.stringify(out, null, 2));
console.log(JSON.stringify(out, null, 2));
console.log('wrote tmp-npb-saved-shadow-overlap.json');
