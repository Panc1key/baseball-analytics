/**
 * 補齊 2025 特徵列缺先發身份（boxscore → postgame_actual_oracle）
 * 不改選注常數；縮回測空白日主因 no_pitcher_id。
 *
 * 用法: node scripts/backfillMlb2025PitcherIdentity.mjs [--limit=50]
 */
import fs from 'fs';
...
const fromDate = argValue('from', '2025-03-01');
...
const out = {
  ...
  note: '補的是 oracle 身分，不假裝 pit_probable；正式選注常數未改',
};

fs.writeFileSync(
  new URL('../tmp-backfill-2025-pitcher-identity.json', import.meta.url),
