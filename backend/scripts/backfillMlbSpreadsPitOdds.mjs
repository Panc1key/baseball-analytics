/**
 * 回補 MLB 開賽前 PIT 讓分（spreads），合併進既有 h2h／totals 快照。
 *
 * 不可另存 spreads-only（會蓋掉 resolvePitOdds 選中的最後一張）。
 *
 * 用法：
 *   node scripts/backfillMlbSpreadsPitOdds.mjs --from=2024-04-01 --to=2024-09-30 --dry-run
 *   node scripts/backfillMlbSpreadsPitOdds.mjs --from=2024-04-01 --to=2024-09-30 --reserve=500
 *   node scripts/backfillMlbSpreadsPitOdds.mjs --from=2025-04-01 --to=2025-09-30 --reserve=500
 */
...
const markets = argValue('markets', 'spreads');
...
function hasSpreads(books) {
  return books.some((b) => (b.markets || []).some((m) => m.key === 'spreads'));
}

function mergeSpreads(existingBooks, incomingBooks) {
  const byKey = new Map(
    existingBooks.map((b) => [
      String(b.key || b.title || ''),
      { ...b, markets: [...(b.markets || [])] },
    ])
  );
  for (const book of incomingBooks || []) {
    ...
    if (!byKey.has(key)) {
      byKey.set(key, {
        key: book.key,
        title: book.title,
        markets: [spreadsMarket],
      });
      continue;
    }
    const target = byKey.get(key);
    const without = (target.markets || []).filter((m) => m.key !== 'spreads');
    target.markets = [...without, spreadsMarket];
  }
  return [...byKey.values()];
}
  ...
  JSON.stringify(
    {
      ...
      needSpreadsMerge: need.length,
      snapshotDays: days.length,
      estimatedCreditsIfAllDays: estimatedCredits,
      note: '合併 spreads 進既有 PIT 快照；不另存 spreads-only',
    },
    null,
    2
  )
...
const upd = db.prepare(`
  ...
  SET bookmakers_json = ?, source = CASE
    WHEN source LIKE '%+spreads%' THEN source
    ELSE source || '+spreads'
  END
  WHERE id = ?
`);
...
for (const day of days) {
  ...
  try {
    ...
    for (const g of list) {
      ...
      const merged = mergeSpreads(g.books, best.bookmakers);
      if (!hasSpreads(merged)) {
        unmatchedGames += 1;
        continue;
      }
...
const outPath = `tmp-backfill-${tag}-spreads-result.json`;
