/**
 * 正式底座上把 normal×away 收縮權重從 0.35 重設為候選值（從 raw 一次收縮，非二次疊加）
 *   node scripts/auditMlbMuWRetarget.mjs
 * 產物: tmp-mu-w-retarget.json
 */
import fs from 'fs';
...
console.log('[mu-w-retarget] build…');
...
function evalW(wMu, basePicks) {
  const picks = selectDaily(selectEligible(rebuild(wMu)));
  const s = summarize(picks);
  const b = summarize(basePicks);
  ...
  for (const leave of ['2024', '2025', '2026']) {
    ...
    const c = summarize(basePicks.filter((x) => keep.includes(x.year)));
    loy[leave] = Number((a.usd50 - c.usd50).toFixed(2));
  }
  const months = [...new Set(raw.map((g) => g.month))].sort();
  let monthSum = 0;
  const monthRows = [];
  for (const m of months) {
    const a = summarize(picks.filter((x) => x.month === m));
    const c = summarize(basePicks.filter((x) => x.month === m));
    const d = Number((a.usd50 - c.usd50).toFixed(2));
    monthSum += d;
    if (d !== 0) monthRows.push({ m, d, baseN: c.bets, altN: a.bets });
  }
  let expUsd = 0;
  ...
  const yearOk =
    ...
    (byYear['2025'] ?? -999) >= -80 &&
    (byYear['2026'] ?? -999) >= -80;
  const fixed = dUsd >= 50 && (dHrPp ?? -1) >= -0.2 && yearOk;
  const stress =
    fixed &&
...
const basePicks = selectDaily(selectEligible(rebuild(0.35)));
const grid = [0.35, 0.38, 0.4, 0.42, 0.45].map((w) => evalW(w, basePicks));
const bestStress =
  grid
    .filter((r) => r.stress && r.wMu !== 0.35)
    .sort((a, b) => b.dUsd - a.dUsd)[0] || null;

const out = {
  experimentId: 'mu-w-retarget-2026-08-08',
  baselineW: 0.35,
  baseline: summarize(basePicks),
  grid,
  bestStress,
  verdict: bestStress
    ? `PASS_MAY_RAISE_W_TO_${bestStress.wMu}`
    : 'KEEP_W_035',
};
fs.writeFileSync(
  new URL('../tmp-mu-w-retarget.json', import.meta.url),
  ...
  JSON.stringify(
    {
      ...
      grid: grid.map((r) => ({
        ...
        fixed: r.fixed,
        stress: r.stress,
        monthRows: r.monthRows,
      })),
      bestStress: bestStress
        ? {
            wMu: bestStress.wMu,
            dUsd: bestStress.dUsd,
            dHrPp: bestStress.dHrPp,
            byYear: bestStress.byYear,
            monthRows: bestStress.monthRows,
          }
        : null,
      verdict: out.verdict,
    },
